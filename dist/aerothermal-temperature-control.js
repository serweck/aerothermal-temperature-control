...
<defs>
    ...
    `}_renderPresets(t){const e=this.config.presets??St;return q`
      <div class="presets">
        ${e.map(e=>q`
            <button
            ...
            >
              ${e.icon?q`<ha-icon icon=${e.icon}></ha-icon>`:G}
              <span>${e.label}</span>
            </button>
          `)}
      </div>
    `}_renderModes(){const t=this.config.show_modes??["off","cool","heat"];return q`
      <div class="modes">
        ${t.map(t=>{const e=Ct[t];if(!e)return G;const s=this.modeState===e.option;return q`
            <button
              class="mode ${s?"active":""}"
              title=${e.label}
